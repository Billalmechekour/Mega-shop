...
export default function OrderHistoryPage() {
  const [orders, setOrders] = useState<Order[]>([
    ...
    },
    {
      ...
      total: 18000,
      items: [
        { productName: "Produit 5", quantity: 3, color: "Rouge", size: "L", price: 6000 },
      ],
    },
    {
      id: 5,
      ...
      total: 12000,
      items: [
        { productName: "Produit 6", quantity: 2, color: "Vert", size: "M", price: 6000 },
      ],
    },
    {
      id: 6,
      ...
      total: 9000,
      items: [
        { productName: "Produit 7", quantity: 1, color: "Jaune", size: "S", price: 9000 },
      ],
    },
  ]);
            ...
            <div className="w-full lg:w-3/5 bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <table className="w-full divide-y divide-gray-200">
                <thead className="bg-black text-white">
                  <tr>
                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs sm:text-sm font-semibold">ID</th>
                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs sm:text-sm font-semibold">Nom du Vendeur</th>
                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs sm:text-sm font-semibold">Date</th>
                    <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs sm:text-sm font-semibold">Total</th>
                  </tr>
                </thead>
              </table>
                ...
                <table className="w-full divide-y divide-gray-200">
                  <tbody className="divide-y divide-gray-200">
                    {filteredOrders.map((order) => (
                      ...
                      >
                        <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900">#{order.id}</td>
                        <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900">
                          <div>
                            ...
                            <p className="text-xs text-gray-600">{order.sellerEmail}</p>
                          </div>
                        </td>
                        <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900">{order.date}</td>
                        <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-semibold text-green-600">{order.total.toFixed(2)} DA</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            >
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex justify-between items-center border-b border-gray-200 pb-3 sm:pb-4">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900">Commande #{selectedOrder.id}</h3>
                </div>

                {/* Informations vendeur */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 bg-gray-50 p-3 sm:p-4 rounded-xl">
                  <div>
                    <span className="text-gray-700 font-medium text-sm sm:text-base">Nom du Vendeur :</span>
                    <span className="text-gray-900 font-semibold block text-sm sm:text-base">{selectedOrder.sellerName}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-sm sm:text-base">Email :</span>
                    <span className="text-gray-900 block text-sm sm:text-base">{selectedOrder.sellerEmail}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-sm sm:text-base">Date :</span>
                    <span className="text-gray-900 block text-sm sm:text-base">{selectedOrder.date}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-sm sm:text-base">Articles :</span>
                    <span className="text-gray-900 font-semibold block text-sm sm:text-base">{selectedOrder.items.length}</span>
                  </div>
                </div>
                    ...
                    {selectedOrder.items.map((item, index) => (
                      ...
                      >
                        <div className="flex-1">
                          <span className="text-gray-900 font-medium text-sm sm:text-base">{item.productName}</span>
                ...
                <div className="bg-green-50 p-3 sm:p-4 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <span className="text-green-700 font-medium text-base sm:text-lg">Total de la commande :</span>
