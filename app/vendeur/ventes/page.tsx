...
import { Store, Download, Filter, X } from "lucide-react";
...
export default function SalesPage() {
  ...
  const handleExport = () => {
    ...
    worksheet["!cols"] = [5, 20, 10, 30, 20, 25, 15, 15].map(w => ({ wch: w }));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col sm:flex-row items-center gap-3 w-full">
          ...
          <div className="relative w-full sm:w-32">
            ...
            >
              {sortConfig?.key === "productName" ? (sortConfig.direction === "asc" ? "A-Z" : "Z-A") : "Produit"}
            </button>
            {isProductOpen && (
              <motion.div
              ...
              >
                <button
                  onClick={() => { handleSort("productName", ""); setIsProductOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  Produit
                </button>
                <button
                  onClick={() => { handleSort("productName", "asc"); setIsProductOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  A-Z
                </button>
                <button
                  onClick={() => { handleSort("productName", "desc"); setIsProductOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  Z-A
                </button>
          ...
          <div className="relative w-full sm:w-32">
            <button
              ...
              className="w-full px-4 py-2 sm:py-3 bg-gradient-to-r from-white to-gray-100/80 backdrop-blur-md border border-gray-300 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.1)] text-gray-800 text-xs sm:text-sm hover:shadow-[0_6px_25px_rgba(0,0,0,0.15)] transition-all duration-300 transform hover:-translate-y-1"
            >
              {sortConfig?.key === "quantity" ? (sortConfig.direction === "asc" ? "Croissant" : "Décroissant") : "Quantité"}
            </button>
            {isQuantityOpen && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
              ...
              >
                <button
                  onClick={() => { handleSort("quantity", ""); setIsQuantityOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  Quantité
                </button>
                <button
                  onClick={() => { handleSort("quantity", "asc"); setIsQuantityOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  Croissant
                </button>
                <button
                  onClick={() => { handleSort("quantity", "desc"); setIsQuantityOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  Décroissant
                </button>
            ...
            filteredSales.map((sale) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  />
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900">{sale.productName}</h3>
                    <p className="text-xs text-gray-600">{sale.clientName}</p>
                  </div>
                </div>
                <div className="text-xs text-gray-600 mb-1">Quantité: {sale.quantity}</div>
                <div className="text-xs text-gray-600 mb-1">Date: {sale.date}</div>
                <div className="text-xs font-semibold text-green-600">Total: {sale.total.toFixed(2)} DA</div>
              </div>
            ))
          ) : (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-black text-white">
                <tr>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold">Image</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold">Produit</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold">Quantité</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden md:table-cell">Couleurs</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden lg:table-cell">Tailles</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden md:table-cell">Client</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden lg:table-cell">Date</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredSales.map((sale) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900">{sale.productName}</td>
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900">{sale.quantity}</td>
                    ...
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900 hidden md:table-cell">
                      <div>
                        ...
                        <p className="text-xs text-gray-600">{sale.clientEmail}</p>
                      </div>
                    </td>
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900 hidden lg:table-cell">{sale.date}</td>
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm font-semibold text-green-600">{sale.total.toFixed(2)} DA</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
                ...
                <div className="flex-grow space-y-2">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900">{selectedSale.productName}</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div className="bg-gray-50 p-2 rounded-lg">
                      <span className="text-gray-700 font-medium text-xs sm:text-sm">Client :</span>
                      <span className="text-gray-900 font-semibold block text-xs sm:text-sm">{selectedSale.clientName}</span>
                    </div>
                    <div className="bg-gray-50 p-2 rounded-lg">
                      <span className="text-gray-700 font-medium text-xs sm:text-sm">Email :</span>
                      <span className="text-gray-900 block text-xs sm:text-sm">{selectedSale.clientEmail}</span>
                    </div>
                    <div className="bg-gray-50 p-2 rounded-lg">
                      <span className="text-gray-700 font-medium text-xs sm:text-sm">Date :</span>
                      <span className="text-gray-900 block text-xs sm:text-sm">{selectedSale.date}</span>
                    </div>
                    <div className="bg-gray-50 p-2 rounded-lg">
                      <span className="text-gray-700 font-medium text-xs sm:text-sm">Quantité :</span>
                      <span className="text-gray-900 font-semibold block text-xs sm:text-sm">{selectedSale.quantity}</span>
                    </div>
                  </div>
                  <div className="bg-green-50 p-2 rounded-lg flex justify-between items-center">
                    <span className="text-green-700 font-medium text-xs sm:text-sm">Total :</span>
                    <span className="text-green-900 font-bold text-base sm:text-lg">{selectedSale.total.toFixed(2)} DA</span>
                  </div>
                  <div className="bg-gray-50 p-2 rounded-lg">
                    <span className="text-gray-700 font-medium block mb-1 text-xs sm:text-sm">Variantes :</span>
