...
export default function VendeurLayout({
  ...
  const navItems = [
    ...
    { name: "Boutique", href: "/vendeur/boutique", icon: ShoppingBag },
    { name: "Ventes", href: "/vendeur/ventes", icon: DollarSign },
    { name: "Commandes", href: "/vendeur/commandes", icon: Package },
    { name: "Notifications", href: "/vendeur/notifications", icon: Bell },
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      {isMobile && (
        <div className="fixed top-0 left-0 right-0 h-16 bg-white z-50 border-b border-gray-200 flex items-center justify-between px-4 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/vendeur/notifications" className="relative">
      ...
      <div
        className={`h-full bg-white flex flex-col shadow-lg z-50 border-r border-gray-200 transition-all duration-300 ${
          isMobile 
            ? `fixed right-0 top-0 bottom-0 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'} w-3/4 max-w-xs`
      ...
      >
        ...
        {(isMobile || !isCollapsed) && (
          <div className={`${isMobile ? 'h-16' : ''} flex items-center justify-between px-4 py-3 border-b border-gray-200`}>
            {isMobile ? (
              <h2 className={`${montserrat.className} text-lg font-bold`}>Menu</h2>
            ) : (
          ...
          <div className="mx-3 my-4 bg-gradient-to-r from-gray-100 to-gray-50 rounded-lg p-3 shadow-sm border border-gray-200 transition-all duration-300">
            <div className="flex items-start">
              ...
              <div className="relative flex-shrink-0">
                <div className={`${isMobile ? 'w-12 h-12' : 'w-14 h-14'} rounded-full overflow-hidden shadow-lg p-0.5 bg-white`}>
                  <div className="w-full h-full rounded-full overflow-hidden">
                    {vendeurInfo.photoUrl ? (
                      ...
                      <div className="w-full h-full bg-black flex items-center justify-center">
                        <UserCheck className={`${isMobile ? 'h-6 w-6' : 'h-7 w-7'} text-white`} />
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="ml-3 flex-1 overflow-hidden">
                <h3 className={`${montserrat.className} text-sm font-bold text-gray-900 truncate`}>
                  {vendeurInfo.name}
                </h3>
            ...
            return (
              <Link
                ...
                className={`flex items-center ${
                  ...
                  isActive
                    ? "bg-gray-100 text-black font-medium" + (isMobile ? "" : " border-r-4 border-black")
