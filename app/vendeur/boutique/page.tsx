...
export default function ShopPage() {
  const [products, setProducts] = useState<Product[]>([
    ...Array.from({ length: 24 }, (_, i) => ({
      ...
      quantity: 30,
      category: ["Vêtements", "Chaussures", "Accessoires"][Math.floor(Math.random() * 3)],
      genre: ["Homme", "Femme", "Enfant", "Unisexe"][Math.floor(Math.random() * 4)],
      price: parseFloat((Math.random() * 100).toFixed(2)),
      image: ["Vêtements", "Chaussures", "Accessoires"][Math.floor(Math.random() * 3)] === "Vêtements"
        ? "https://images.unsplash.com/photo-1542291026-7eec264c27ff?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80"
        : ["Vêtements", "Chaussures", "Accessoires"][Math.floor(Math.random() * 3)] === "Chaussures"
        ? "https://images.unsplash.com/photo-1542291026-7eec264c27ff?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80"
  ...
  const [stockDetailInput, setStockDetailInput] = useState({ size: "", color: "", quantity: 0 });
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [productToDelete, setProductToDelete] = useState<number | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [showDetailsModal, setShowDetailsModal] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [stockAdjustMode, setStockAdjustMode] = useState<{ [key: string]: "increment" | "decrement" | null }>({});
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, isMainImage: boolean) => {
  ...
  const availableColors = [
    "Noir", "Blanc", "Rouge", "Bleu", "Vert", "Jaune", "Orange", "Rose", "Gris", "Marron", "Violet", "Beige"
  ];

  const categories = [
    { nom: "Hauts", description: "T-shirts, chemises, pulls et autres vêtements pour le haut du corps." },
    { nom: "Bas", description: "Pantalons, jeans, shorts et autres vêtements pour le bas du corps." },
    { nom: "Robes & Ensembles", description: "Robes et ensembles assortis pour toutes occasions." },
    { nom: "Vestes & Manteaux", description: "Vestes légères, manteaux d'hiver et blousons." },
    { nom: "Chaussures", description: "Baskets, bottes, sandales et autres types de chaussures." },
    { nom: "Accessoires", description: "Sacs, écharpes, ceintures et autres compléments de tenue." },
    { nom: "Autres", description: "Articles divers ne rentrant pas dans les autres catégories." },
  ];
              ...
              >
                ...
                <div className="p-3 sm:p-4">
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <p className="text-xs sm:text-sm text-gray-600">
                      <span className="font-bold">Quantité :</span> {product.quantity}
                    </p>
                  </div>
              ...
              <form onSubmit={handleAddProduct} className="space-y-4 sm:space-y-6">
                {!editingProduct ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-gray-600 text-sm sm:text-base">DA {newProduct.price.toFixed(2)}</p>
                    </div>
                    <div>
                      <label className="block text-xs sm:text-sm font-medium text-gray-800">
                        Nouveau prix (DA)
                      ...
                      {newProduct.discountPercentage && (
                        <p className="mt-2 text-xs sm:text-sm text-green-600">
                          Réduction : {newProduct.discountPercentage}% par rapport au prix initial
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-1 space-y-2">
                    {editingProduct && (
                      <div className="mb-2">
                        <span className="font-bold text-xs sm:text-sm">Quantité Totale : </span>
                        <span className="text-xs sm:text-sm">{newProduct.stockDetails.reduce((sum, detail) => sum + detail.quantity, 0)}</span>
                      </div>
                    )}
                    <div className="flex flex-col sm:flex-row gap-2">
                      <select
                        ...
                        onChange={(e) =>
                          setStockDetailInput({ ...stockDetailInput, size: e.target.value })
                        }
                        className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-black focus:border-black transition-all duration-200 text-gray-800 placeholder-gray-400 text-sm sm:text-base"
                      >
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setStockDetailInput({ ...stockDetailInput, color: e.target.value })
                        }
                        className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-black focus:border-black transition-all duration-200 text-gray-800 placeholder-gray-400 text-sm sm:text-base"
                      >
                            ...
                            {editingProduct ? (
                              <div className="flex items-center gap-2">
                                  ...
                                  {mode && (
                                    <>
                                      <button
                                        ...
                                        className={`${mode === "increment" ? "bg-green-500" : "bg-red-500"} text-white px-2 py-1 rounded-lg hover:${mode === "increment" ? "bg-green-600" : "bg-red-600"} text-xs sm:text-sm`}
                                      >
                                        1
                                      </button>
                                      <button
                                ...
                                <div className="flex flex-col gap-1">
                                  <button
                                    type="button"
                                    onClick={() => setStockAdjustMode({ ...stockAdjustMode, [key]: "increment" })}
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setStockAdjustMode({ ...stockAdjustMode, [key]: "decrement" })}
                              ...
                              <span className="text-xs sm:text-sm">{detail.quantity}</span>
                            )}
                          </div>
                        );
                      })}
          ...
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 animate-fade-in">
            <div className="bg-gradient-to-br from-white to-gray-100 p-4 sm:p-6 rounded-2xl w-full max-w-[90%] sm:max-w-md shadow-2xl transform transition-all duration-500 scale-100 max-h-[90vh] overflow-y-auto border border-gray-200/50">
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Nom du produit</label>
                  <p className="mt-1 text-base sm:text-lg font-medium text-gray-900">{selectedProduct.name}</p>
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Description</label>
                  <p className="mt-1 text-gray-600 leading-relaxed text-sm sm:text-base">{selectedProduct.description}</p>
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Quantité totale</label>
                  <p className="mt-1 text-gray-900 font-medium text-sm sm:text-base">{selectedProduct.quantity}</p>
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Répartition du stock</label>
                  <div className="mt-2 space-y-2">
                    {selectedProduct.stockDetails.length > 0 ? (
                      ...
                      <p className="text-gray-500 text-sm sm:text-base">Aucun détail de stock disponible.</p>
                    )}
                  </div>
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Tailles disponibles</label>
                  <div className="mt-2 flex flex-wrap gap-2">
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Couleurs disponibles</label>
                  <div className="mt-2 flex flex-wrap gap-2">
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Date d’ajout</label>
                  <p className="mt-1 text-gray-600 text-sm sm:text-base">{selectedProduct.dateAdded || "Non spécifiée"}</p>
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700">Dernière modification</label>
                  <p className="mt-1 text-gray-600 text-sm sm:text-base">{selectedProduct.lastModified || "Non spécifiée"}</p>
                </div>
              </div>
