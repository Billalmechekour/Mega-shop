...
export default function ReportsPage() {
  ...
  const handleExport = () => {
    ...
    worksheet["!cols"] = [20, 25, 20, 20, 30, 15, 15, 40].map(w => ({ wch: w }));
        ...
        <div className="mb-8 flex flex-col sm:flex-row items-center gap-3 w-full">
          ...
          <div className="relative w-full sm:w-32">
            <button
              ...
              className="w-full px-4 py-2 sm:py-3 bg-gradient-to-r from-white to-gray-100/80 backdrop-blur-md border border-gray-300 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.1)] text-gray-800 text-xs sm:text-sm hover:shadow-[0_6px_25px_rgba(0,0,0,0.15)] transition-all duration-300 transform hover:-translate-y-1"
            >
              {statusFilter === "En attente" ? "En attente" : statusFilter === "Répondu" ? "Répondu" : "Statut"}
            </button>
            {isStatusOpen && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
              ...
              >
                ...
                <button
                  onClick={() => { setStatusFilter("En attente"); setIsStatusOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  En attente
                </button>
                <button
                  onClick={() => { setStatusFilter("Répondu"); setIsStatusOpen(false); }}
                  className="w-full px-4 py-2 text-gray-800 text-xs sm:text-sm hover:bg-gray-100 transition-all duration-200"
                >
                  Répondu
                </button>
            ...
            filteredReports.map((report, index) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-sm font-semibold text-gray-900">{report.clientName}</h3>
                  <span
                ...
                <div className="text-xs text-gray-600 mb-1">Produit: {report.productName}</div>
                <div className="text-xs text-gray-600 mb-1">Vendeur: {report.vendor}</div>
                <div className="text-xs text-gray-600 mb-1">Date: {report.date}</div>
              </div>
            ))
          ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-black text-white">
                <tr>
                  ...
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden md:table-cell">Produit</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden lg:table-cell">Vendeur</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold hidden md:table-cell">Date</th>
                  <th className="px-4 py-2 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold">Statut</th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900">
                      <div>
                        ...
                        <p className="text-xs text-gray-600">{report.clientEmail}</p>
                      </div>
                    </td>
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900 hidden md:table-cell">{report.productName}</td>
                    ...
                    <td className="px-4 py-2 sm:px-6 sm:py-4 text-xs sm:text-sm text-gray-900 hidden md:table-cell">{report.date}</td>
            ...
            >
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center border-b border-gray-200 pb-4">
                  <h3 className="text-lg sm:text-2xl font-bold text-gray-900">Signalement</h3>
                  <span
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 bg-gray-50 p-3 sm:p-4 rounded-xl">
                  <div>
                    ...
                    <span className="text-gray-900 font-semibold block text-xs sm:text-sm">{selectedReport.clientName}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-xs sm:text-sm">Email :</span>
                    <span className="text-gray-900 block text-xs sm:text-sm">{selectedReport.clientEmail}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-xs sm:text-sm">Produit :</span>
                    <span className="text-gray-900 block text-xs sm:text-sm">{selectedReport.productName}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-xs sm:text-sm">Vendeur :</span>
                    <span className="text-gray-900 block text-xs sm:text-sm">{selectedReport.vendor}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium text-xs sm:text-sm">Date :</span>
                    <span className="text-gray-900 block text-xs sm:text-sm">{selectedReport.date}</span>
                  </div>
                </div>

                {/* Texte du signalement */}
                <div className="bg-gray-50 p-3 sm:p-4 rounded-xl">
                  <span className="text-gray-700 font-medium block mb-2 text-xs sm:text-sm">Texte du signalement :</span>
                  <p className="text-gray-900 text-xs sm:text-sm">{selectedReport.signalText}</p>
                </div>

                {/* Réponse */}
                {selectedReport.status === "Répondu" ? (
                  <div className="bg-green-50 p-3 sm:p-4 rounded-xl">
                    <span className="text-green-700 font-medium block mb-2 text-xs sm:text-sm">Réponse envoyée :</span>
                    <p className="text-gray-900 text-xs sm:text-sm">{selectedReport.response}</p>
                  </div>
                ) : (
